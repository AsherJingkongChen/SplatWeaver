...
class DatasetRe10k(Dataset):
    ...
    def __init__(
        ...
        with ThreadPoolExecutor(max_workers=64) as executor:
            ...
            for future in as_completed(futures):
                ...
                index += 1
        print(f"Re10K Dataset: {self.stage}: loaded {len(self.scene_ids)} scenes")
    ...
    def getitem(self, index: int, num_context_views: int, patchsize: tuple) -> dict:
        print('we are using re10k.')
        
        scene = self.scene_ids[index]
        example = self.scenes[scene]
        identity_ext = torch.eye(4, dtype=torch.float32)

        extrinsics = identity_ext.unsqueeze(0).repeat(len(example), 1, 1)
        identity_int = torch.tensor([
            [1.0, 0.0, 0.5],
            [0.0, 1.0, 0.5],
            [0.0, 0.0, 1.0]
        ], dtype=torch.float32)

        intrinsics = identity_int.unsqueeze(0).repeat(len(example), 1, 1)
