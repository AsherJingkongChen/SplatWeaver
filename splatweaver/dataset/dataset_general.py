...
@dataclass
class DatasetGeneralCfg(DatasetCfgCommon):
    name: str
    roots: list[Path]
    baseline_min: float
    baseline_max: float
...
@dataclass
class DatasetGeneralCfgWrapper:
    general: DatasetGeneralCfg
    ...
    def __init__(
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            for future in as_completed(futures):
                ...
                index += 1
        print(f"General Dataset: {self.stage}: loaded {len(self.scene_ids)} scenes")
