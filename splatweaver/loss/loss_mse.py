...
import torch
from splatweaver.dataset.types import BatchedExample
from splatweaver.model.decoder.decoder import DecoderOutput
from splatweaver.model.types import Gaussians
...
class LossMse(Loss[LossMseCfg, LossMseCfgWrapper]):
    def forward(
        ...
        return self.cfg.weight * torch.nan_to_num((delta**2).mean(), nan=0.0, posinf=0.0, neginf=0.0)  #if global_step < 6000 else self.cfg.weight * torch.nan_to_num((delta.abs()).mean(), nan=0.0, posinf=0.0, neginf=0.0)#+ 0.3 * ssim(pred_img, gt_image) #abs()
