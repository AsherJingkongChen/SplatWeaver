...
from loss.loss_expert import LossExpert
from model.encoder.vggt.utils.pose_enc import pose_encoding_to_extri_intri

from ..loss.loss_distill import DistillLoss
from splatweaver.utils.render import generate_path
from splatweaver.utils.point import get_normal_map
...
def save_chw_tensor_as_image(tensor, save_path):
    ...
    if img_np.ndim != 3:
        raise ValueError(f"Expected 3D array (C, H, W), got shape {img_np.shape}")
    
    C, H, W = img_np.shape
    if C not in [1, 3]:
        raise ValueError(f"Only support 1 (grayscale) or 3 (RGB) channels, got C={C}")
    ...
    if C == 1:
        img_hwc = img_hwc.squeeze(axis=2)  # (H, W)

    if C == 1:
        pil_img = Image.fromarray(img_hwc, mode='L')
    else:  # C == 3
...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #print('batch context image: ', batch["context"]["image"].shape)
        ...
        gs_choice = encoder_output.choice
        gs_logit = encoder_output.logit
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if gs_choice is not None:
                total_loss = total_loss + LossExpert(gs_choice, gs_logit, target_gt,self.global_step)
