...
from splatweaver.dataset.shims.bounds_shim import apply_bounds_shim
from splatweaver.dataset.shims.normalize_shim import apply_normalize_shim
from splatweaver.dataset.shims.patch_shim import apply_patch_shim
from splatweaver.dataset.types import BatchedExample, DataShim
from splatweaver.geometry.projection import sample_image_grid

from splatweaver.model.encoder.heads.splatweaver_head import SplatWeaver_Head
from splatweaver.model.encoder.vggt.utils.geometry import (
...
from splatweaver.utils.geometry import get_rel_pos  # used for model hub
...
from splatweaver.model.encoder.heads.head_modules import TransformerBlockSelfAttn
from splatweaver.model.encoder.vggt.heads.dpt_head import DPTHead
from splatweaver.model.encoder.vggt.layers.mlp import Mlp
from splatweaver.model.encoder.vggt.models.vggt import VGGT
...
class GSHeadParams:
    ...
    feature_dim: int = 128*2
    depth_mode = ("exp", -inf, inf)
    conf_mode = True


@dataclass
class EncoderSplatWeaverCfg:
    name: Literal["splatweaver"]
    anchor_feat_dim: int
    d_feature: int
...
class EncoderSplatWeaver(Encoder[EncoderSplatWeaverCfg]):
    backbone: nn.Module
    gaussian_adapter: GaussianAdapter

    def __init__(self, cfg: EncoderSplatWeaverCfg) -> None:
        super().__init__(cfg)
        ...
        self.raw_gs_dim = 3 + 1 + self.gaussian_adapter.d_in  # 1 for opacity

        self.gs_params_head_type = cfg.gs_params_head_type
        # fake backbone for head parameters
        head_params = GSHeadParams()
        self.gaussian_param_head = SplatWeaver_Head(
            dim_in=2048,
